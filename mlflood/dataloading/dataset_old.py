...
#         return waterdepth, rainfall_events, timesteps, dem, mask, event_name
    

def load_dataset(catchment_num = "toy", **catchment_kwargs):
    train_dataset = MyCatchment(PATH_GENERATED / Path(catchment_num+"-train.h5"), **catchment_kwargs)
    valid_dataset = MyCatchment(PATH_GENERATED / Path(catchment_num+"-val.h5"), **catchment_kwargs)
    return train_dataset, valid_dataset

# def load_test_dataset(catchment_num = "sims", **catchment_kwargs):
#     dataset = MyCatchment(PATH_GENERATED / Path(catchment_num+"-test.h5"), **catchment_kwargs)
#     return dataset
...
class MyCatchment(torch.utils.data.Dataset):
    ...
    def __init__(self,  h5file, tau=0.5, upsilon=0, timestep=1, sample_type="single", dim_patch=64, fix_indexes=False, border_size=0, normalize_output = False, use_diff_dem=False, num_patch = 10, predict_ahead = 0):
        'Initialization'
        ...
        self.input_channels = 3 *timestep + (4 if use_diff_dem else 0)
...
#         self.inds = self.get_all_fix_indexes()
    ...
    def __getitem__(self, index):
        ...
        inputs = self.build_inputs(xin, self.rainfall_events[index_e][index_t:index_t+self.timestep], mask, dem, diff_dem)
        return inputs, outputs
    
#         return inputs, y, {'index_e':index_e, 'timestep':index_t, 'x_p': x_p, 'y_p':y_p}
        ...
        # if self.timestep>1:
        #     x[-self.timestep+1:] = normalize(torch.diff(xin, axis=0))
        # mask = mask.unsqueeze(0).clone()
        # return ((x, mask), y)
        
        # return {'data': x, 'mask': mask, 'index_e':index_e, 'timestep':index_t, 'index_s':index_s, 'x_p': x_p, 'y_p':y_p} , y
    ...
    def build_inputs(self, xin, rainfall_events, mask, dem, diff_dem):
        ...
        '''
        rain_t = torch.zeros((self.timestep, *dem.shape))
        ...
        if self.use_diff_dem:
            x[1:5] = diff_dem
        x[1+d : self.timestep+1+d ] = rain_t
        x[self.timestep+1+d : 2*self.timestep+1+d] = xin
        if self.timestep>1:
            x[2*self.timestep+1+d:] = normalize(torch.diff(xin, axis=0))            
