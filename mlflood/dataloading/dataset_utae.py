...
def load_dataset_utae(catchment_kwargs):
...
def dataloader_args_utae(train_dataset, valid_dataset, catchment_num = "toy", batch_size = 8):
...
def dataloader_args_utae_test(train_dataset, catchment_num = "toy", batch_size = 8):
...
class MyCatchment(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        xout = self.waterdepth[index_e][index_t + self.timestep + self.predict_ahead] # for specifying how much ahead output prediction
        mask = self.dem_mask
        dem = self.dem
    ...
    def build_inputs(self, xin, rainfall_events, mask, dem, diff_dem):
        ...
        for i in range(self.timestep + tsh):    
        ...
        if self.use_diff_dem:
            temp_dem = torch.zeros((d+1, *dem.shape))
            temp_dem[0] = dem.clone()
            temp_dem[1:5] = diff_dem
        else:
            temp_dem = dem.clone().unsqueeze(0)
        inp = []
        for p in range(self.timestep):
            temp_r = rain_t[p:p+tsh+1] ## added predict_ahead rainfall data as channels
            temp_wd = xin[p].unsqueeze(0)
            #temp_com = torch.stack((temp_dem, temp_r, temp_wd), dim=0)
            temp_com = torch.vstack((temp_dem, temp_r, temp_wd))
            inp.append(temp_com)
        inp2 = torch.stack(inp)
        ###################################      
