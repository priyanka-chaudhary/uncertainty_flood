...
def predict_event(model, dataset, event_num, arch, start_ts=None, ar = True, T = None):
    ...
    recons_pred_full_sigma = torch.zeros(dataset.px - 2*b, dataset.py - 2*b) 
    ...
    for inds_count, (x_p, y_p) in enumerate(inds):
        ...
        y_pred = model(data)
        sigma = y_pred['sigma'].squeeze().detach().cpu()
        y_pred = y_pred['y_pred'].squeeze().detach().cpu()
        mask1 = mask1.squeeze().detach().cpu()
        ...
        # recons_pred_full_sigma[x_p:x_p+patch_dim, y_p:y_p + patch_dim] = sigma
        ...
        recons_pred_full_sigma[x_p + 10:
                    x_p + 10 + (int(patch_dim/2)),
                    y_p + 10:
                    y_p + 10 + (int(patch_dim/2))] = crop_overlapping_patches(sigma, patch_dim)
